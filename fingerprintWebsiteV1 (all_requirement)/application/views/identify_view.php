...
<script>
				...
				// saveToDB();
				 // trHtml += '<thead><th> First Name </th><th>File Name</th><th> Score </th><th> ลายนิ้วมือ </th></thead>'
		    		...
		    		$.each(obj, function(key,val){
		    	 		callFingerScores(val['Id'],val['organisation']);
	...
	function callFingerScores(criminal_id,organisation){
		var trHtml;
		$.ajax({
		    		url : "loadImage/"+criminal_id+"/"+organisation,
		    		type : "post",
		    		
		}).success(function(result){
			alert(result);
			 var obj = JSON.parse(result);
		//	 var obj2 = JSON.parse(obj['scoreAndImage']);
			 trHtml += '<thead><th> First Name </th><th>File Name</th><th> Score </th><th> ลายนิ้วมือ </th></thead>';
			 // $.each(obj, function(key,val){
		    	 		// callFingerScores(val['Id'],val['organisation']);
		    	 		// alert(obj["id"]);
		    			trHtml += '<tr><td class="firstName_result">' + obj['id'] +'</td>'+
		    			'<td >' + obj['FingerMatchesImages'] + '</td>' +
		    			'<td class="score_result">' + obj['scoreAndImage']+  '</td>' + 
		    			// '<td><a href="loadImage/'+ val['Id']+'"> ดู </a>'+ '</td>' + 
		    			'</tr>'
		    			$('#divTable').css("display","inline");
		    		$('#tableResult').append(trHtml);
