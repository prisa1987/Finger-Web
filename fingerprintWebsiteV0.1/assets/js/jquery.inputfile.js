...
(function ($) {
	...
	function init(params) {
		...
		this.each(function () {
			...
			// $link = $('<label style="color: #dcdcdc;font-size: 20px;">เลือกแล้ว</label>').appendTo($previous);
			$link = $('').appendTo($previous);
            if (!params.dontRemove) {
                // $remove = $('<button class="' + REMOVE_CLASS + '"></button>').appendTo($previous);
